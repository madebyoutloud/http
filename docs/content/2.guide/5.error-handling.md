...
```ts
import { errors } from '@outloud/http'
...
try {
  const data = await http.$get('/users', {}, { timeout: 1000 })
} catch (error) {
...
```ts
try {
  await http.$get('/users/999', { search: 'test' })
} catch (error) {
  if (http.isError(error)) {
...
try {
  ...
  if (!http.isError(error)) throw error
...
try {
  await http.$post<User, CreateUserDto>('/users', {
    name: '',
    email: 'invalid'
  })
} catch (error) {
  if (http.isError(error) && error.status === 422) {
...
```ts
import { createClient } from '@outloud/http'
...
1. **Always check error types**: Use `http.isError()` or `instanceof` to type-check errors
